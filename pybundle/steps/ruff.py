...
def _repo_has_py_files(root: Path) -> bool:
    ...
    for p in root.rglob("*.py"):
        ...
        parts = set(p.parts)
        if ".venv" in parts or "__pycache__" in parts or ".mypy_cache" in parts or ".ruff_cache" in parts:
            continue
        if "node_modules" in parts or "dist" in parts or "build" in parts or "artifacts" in parts:
            continue
        return True
    return False
...
class RuffCheckStep:
    ...
    def run(self, ctx: BundleContext) -> StepResult:
        ...
        ruff = which("ruff")
        if not ruff:
            out.write_text("ruff not found; skipping (pip install ruff)\n", encoding="utf-8")
            return StepResult(self.name, "SKIP", 0, "missing ruff")

        if not _repo_has_py_files(ctx.root):
            out.write_text("no .py files detected; skipping ruff check\n", encoding="utf-8")
...
class RuffFormatCheckStep:
    ...
    def run(self, ctx: BundleContext) -> StepResult:
        ...
        if not _repo_has_py_files(ctx.root):
            out.write_text("no .py files detected; skipping ruff format check\n", encoding="utf-8")
            return StepResult(self.name, "SKIP", 0, "no python files")
